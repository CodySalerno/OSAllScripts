...
@ScriptManifest(name = "Runite Blast Furnace", author = "Iownreality1", info = "Creates bar", version = 0.1, logo = "")
public class BlastFurnaceRunite extends Script {
    ...
    public void onPaint(final Graphics2D g)
    {
        ...
        g.drawString("Bars Created  " + (int)(getExperienceTracker().getGainedXP(Skill.SMITHING)/50), 10,330);
        g.drawString("Bars per hour  " + (int)(getExperienceTracker().getGainedXPPerHour(Skill.SMITHING)/50), 10,350);
    }
    ...
    public int onLoop() throws InterruptedException
    {
        ...
        if (configs.get(547) < 108 && blastFurnace.contains(myPosition()) && !objects.closest("Bar dispenser").hasAction("Take"))
        {
            useCoal();
            return (random(400,700));
        }
        if (blastFurnace.contains(myPosition()))
        {
            ...
            if (objects.closest("Bar dispenser").hasAction("Take")) //bars are ready need to take
            {
                if (inventory.onlyContains("Coal bag"))
                {
                    ...
                    if (widgets.get(270,14,38) != null)
                    {
                        ...
                        Sleep.sleepUntil(() -> inventory.contains("Runite bar"), 1000);
            ...
            {
                log("in else");
                if (inventory.contains("Runite bar"))
                {
                    log("Banking bars");
                    if (!bank.isOpen())
                    {
                    ...
                    if (bank.isOpen() && inventory.contains("Runite bar"))
                    {
                        if (!bank.contains("Runite ore") || !bank.contains("Coal"))
                        {
                            //stop();
                        ...
                        bank.depositAll("Runite bar");
                        Sleep.sleepUntil(() -> !inventory.contains("Runite bar"), 3000);
                        staminaCheck();
                    }
                }
                ...
                if (bank.isOpen() && inventory.onlyContains("Coal bag") && configs.get(547) >= 108)
                {
                    if (!bank.contains("Runite ore") || !bank.contains("Coal"))
                    {
                        //stop();
                    ...
                    bank.withdrawAll("Runite ore");
                    walking.webWalk(bottomConveyor);
                }
                if (inventory.contains("Runite ore") &&  (!nearConveyor.contains(myPosition())) && (!objects.closest("Bar dispenser").hasAction("Take")))
                {
                    log("walking with ore to conveyor");
                    walking.webWalk(nearConveyor);
                    //betterWalk.MyWalkingEvent(bottomConveyor);
                }
                if (nearConveyor.contains(myPosition()) && inventory.contains("Runite ore") && (!objects.closest("Bar dispenser").hasAction("Take")))
                {
                    ...
                    if (menu.isOpen())
                    {
                        ...
                        {
                            Sleep.sleepUntil(() -> !inventory.contains("Runite ore"), 5000);
                ...
                if (!inventory.contains("Runite ore") && !coalBagEmptied && conveyorPosition.equals(myPosition()))
                {
                ...
                if (nearConveyor.contains(myPosition()) && !inventory.contains("Runite ore") && coalBagEmptied)
                {
                    log("no Runite ore going to bar position");
                    walking.webWalk(barPosition);
                ...
                if ((!bank.isOpen() && !inventory.isEmptyExcept("Runite ore", "Runite bar", "Coal", "Coal bag") ))
                {
    ...
    private boolean getMaterials() throws InterruptedException, IOException {
        ...
        if (blastFurnace.contains(myPosition()))
        {
            if (bank.isOpen())
            {
                ...
                {
                    log("Withdraw varrock teleport, Runite bars, and coins to resupply.");
                    bank.withdraw("Varrock teleport", 1);
                    if (bank.contains("Runite bar"))
                    {
        ...
        else if (UsefulAreas.GeArea.contains(myPosition()))
        {
            if (!checkedBank)
            {
                ...
                if (bank.isOpen())
                {
                    ...
                    if (!bank.contains("Coins") && !bank.contains("Runite bar"))
                    {
                        checkedBank = true;
                        return true;
                    }
            ...
            else if (grandExchange.isOpen())
            {
                ...
                if (inventory.contains("Runite bar"))
                {
                    log("selling Runite bars");
                    grandExchange.sellItem(2364,12500,(int)inventory.getAmount("Runite bar"));
                }
                else if (steelBarPending() || steelBarInitializing())
                {
                    log("waiting for Runite bars to sell.");
                ...
                else if (grandExchange.getStatus(GrandExchange.Box.BOX_1) == GrandExchange.Status.FINISHED_SALE)
                {
                    ...
                    sleep(random(1800,2400));
                        ...
                        && !inventory.contains("Runite bar") && inventory.contains("Coins") && !inventory.contains("Runite ore"))
                {
                    log("no Runite bars, and box 1 is empty. time to buy rune.");
                    int coins = (int)inventory.getAmount("Coins");
                    if (amountOfOre == 0)
                    {
                        amountOfOre = coins/11300;
                        if (amountOfOre > 1500)
                        {
                            amountOfOre = 1500;
                        }
                    ...
                    if (amountOfOre != 0)
                    {
                        grandExchange.buyItem(451, "Runite ore", 11300,amountOfOre);
                ...
                {
                    grandExchange.collect();
                    Sleep.sleepUntil(() -> inventory.contains("Coal") && inventory.contains("Runite ore"), 5000);
                }
                else if (inventory.contains("Coal") && inventory.contains("Runite ore"))
                {
                    amountOfOre = 0;
                    return false;
                }
